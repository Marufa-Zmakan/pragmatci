...
class OrderStage(models.Model):
    ...
    def get_order_stages(self, warehouse_id):
        order_stages = self.env['order.stage'].search([('warehouse_ids', '=', warehouse_id)], order="sequence, id")
        return order_stages

    @api.model
    def get_order_stage_index(self, order_stages, order_stage_id):
        try:
            return order_stages.ids.index(order_stage_id)
        except ValueError as e:
            _logger.error(_("Missing order stage warehouse configuration mapping!"))
            _logger.exception(e)
            return 0
